...
rem Builds the byProd raylib sample for the web with Emscripten. See
rem README.md for the full prerequisites, in short an activated emsdk
rem (emcc on PATH), then:
rem
rem     build_web.bat <raylib dir> <byprod include dir> <byprod archive>
rem
rem     <raylib dir>          an unpacked raylib webassembly release
rem     <byprod include dir>  the SDK folder holding byprod\byprod.h
rem     <byprod archive>      the SDK's byProd-wasm32-emscripten.a
...
if "%BPD_ARCHIVE%"=="" (
    echo usage: build_web.bat ^<raylib dir^> ^<byprod include dir^> ^<byprod archive^>
    echo.
    echo     ^<raylib dir^>          an unpacked raylib webassembly release
    echo     ^<byprod include dir^>  the SDK folder holding byprod\byprod.h
    echo     ^<byprod archive^>      the SDK's byProd-wasm32-emscripten.a
    exit /b 1
)
...
if not exist "%BPD_INCLUDE%\byprod\byprod.h" (
    echo build_web: "%BPD_INCLUDE%" does not hold byprod\byprod.h.
    exit /b 1
)

if not exist "%BPD_ARCHIVE%" (
    echo build_web: "%BPD_ARCHIVE%" is not there. Pass the path to
    echo byProd-wasm32-emscripten.a from a byProd SDK or build.
    exit /b 1
)

if not exist "..\assets\sample_project.byprod" (
    echo build_web: the shared samples\assets folder is missing
    echo sample_project.byprod, see README.md.
    exit /b 1
)
...
call emcc -c main.c -Os -Wall ^
  -I"%BPD_INCLUDE%" -I"%RAYLIB_DIR%\include" ^
  -o build\main.o
if errorlevel 1 exit /b 1

rem em++ rather than emcc: byProd is C++ internally, so the link needs
...
call em++ build\main.o "%BPD_ARCHIVE%" "%RAYLIB_LIB%" ^
  -Os ^
  -sUSE_GLFW=3 ^
